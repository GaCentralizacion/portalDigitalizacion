registrationModule.controller("nodoController", function($scope, $rootScope, $location, utils, localStorageService, alertFactory, nodoRepository, documentoRepository, alertaRepository, empleadoRepository, searchRepository, globalFactory) {
    ...
    var getEmpleadoSuccessCallback = function(data, status, headers, config) {
        ...
        if ($rootScope.empleado != null) {
            ...
            console.log('folio: ' + $rootScope.folio)

            if ($rootScope.folio == null) {
                console.log('$rootScope.folio == null!!!!!!!') //LQMA 02052017
    ...
    $scope.navegaFolio = function(folio) {
        ...
        console.log('tipoFolio: ' + $rootScope.tipoFolio)
        console.log('folio antes de asignar click modal: ' + $rootScope.folio);
        console.log('folio parametro: ' + folio)

        console.log('$scope.nodoClick: ' + $rootScope.nodoClick)
        console.log('$scope.nodoClickDestino: ' + $rootScope.nodoClickDestino)
    ...
    $rootScope.LoadActiveNode = function() {
        if ($scope.currentNode.estatus != 1) {
            ...
            Apply();
            //--------------------------------------------------------------------------------
            //--Verifico si es el nodo 15 Obtengo el resumen de los procesos 
            //--------------------------------------------------------------------------------
            if ($scope.currentNode.id == 15) {
                documentoRepository.getProceso($scope.currentNode.folio, $scope.idProceso).then(function(result) {
    ...
    var getNavegacionSuccessCallback = function(data, status, headers, config) {
        ...
        //console.log('getNavegacionSuccessCallback -- $scope.currentNode: ' + $scope.currentNode);
        //$scope.nodoClick = $scope.currentNode;        
    ...
    var getNavegacionServicioSuccessCallback = function(data, status, headers, config) {
        ...
        //Asigno titulo de modal
        //console.log('getNavegacionServicioSuccessCallback -- $scope.currentNode: ' + $scope.currentNode);
        //$scope.nodoClick = $scope.currentNode;
    ...
    /////////////////////////////////////////////////////////////////////////////////////////////////////////
    //                           Obtiene el estatus de la unidad para saber si esta apartada
    ///////////////////////////////////////////////////////////////////////////////////////////////////////// 
    $rootScope.getUnidadApartada = function(folio) {
        searchRepository.getUnidadApartada(folio).then(function(result) {
            if (result.data.length > 0) {
                $scope.unidadesApartadas = result.data[0];
            } else {
                $scope.unidadesApartadas.idGerenteVentas = -1;
            }

        });

    };
    $scope.cambiaEstatusApartada = function() {
        //console.log('Entre a donde cambiara el estatus')
        nodoRepository.cambiaEstatusApartada($scope.unidadesApartadas.numeroSerie, $rootScope.folio, $scope.unidadesApartadas.idGerenteVentas).then(function(result) {
            var respuesta = result.data[0];
            if (respuesta.estatus == 0) {
                alertFactory.success(respuesta.mensaje)
            } else if (respuesta.estatus == 0) {
                alertFactory.error(respuesta.mensaje)
            } else {
                alertFactory.error('Ocurrio un problema')
            }
        });
    ...
    /////////////////////////////////////////////////////////////////////////////////////////////////////////
    //Verifica si el usuario tiene los permisos para visualizar el boton y si la documentacion esta completa 
    ///////////////////////////////////////////////////////////////////////////////////////////////////////// 
    $rootScope.VerificaUsuario = function(usuario, folio) {
        nodoRepository.verificaUsuario(folio, usuario).then(function(result) {
            console.log(result.data[0], 'Tendre permiso ?')
            $scope.permisoCerrar = result.data[0];
        });
    };
    $scope.CerrarNodo = function(idNodo) {
        console.log($rootScope.folio, 'usuario', $rootScope.empleado.idUsuario, 'proceso', $rootScope.idProceso, 'nodo', idNodo)
        nodoRepository.cierraNodo($rootScope.idProceso, idNodo, $rootScope.folio).then(function(result) {
            nodoRepository.bitacoraAccion($rootScope.folio, idNodo, $rootScope.empleado.idUsuario).then(function(result) {
                console.log(result.data);
                $rootScope.VerificaUsuario($rootScope.empleado.idUsuario, $rootScope.folio);
                $scope.init();
            });
    ...
    $scope.resumenCotizacion = function() {
        ...
        $rootScope.mostrarImprimir = false;
        $scope.limpiaModal();
        $('#resumen').modal('show');
    ...
    $rootScope.buscarResumen = function(factura) {
        ...
        $rootScope.mostrarImprimir = true;
        $rootScope.facturaBusca = factura.factura;
        $rootScope.numeroSerieBusca = factura.numeroSerie;
        $rootScope.idDetCot = factura.idDetalleCotizacion;
        nodoRepository.getResumenCargo(factura.idDetalleCotizacion, $rootScope.folio).then(function(result) {
            $rootScope.loadingPdf = false;
            $rootScope.cargos = result.data;
    ...
    $rootScope.detalleFactura = function(factura) {
        $rootScope.loadingPdf = true;
        $rootScope.mostrarDetalle = false;
        console.log('Si entre al detalle ', factura)
        $rootScope.totalCargos = 0;
        angular.forEach($rootScope.cargos, function(value, key) {
            $rootScope.totalCargos = $rootScope.totalCargos + value.total;
        });
